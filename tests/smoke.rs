...
use vliw_simulator::system::{
    at_most_one_modified_for_system, worst_case_visibility, Bus, BusAccessKind, BusReq, System,
};
...
// ---------------------------------------------------------------------------
// Stage 4C: AcqLoad / RelStore execution
// ---------------------------------------------------------------------------

fn acq_load(dst: usize, base: usize, imm: i64) -> Syllable {
    Syllable {
        opcode: Opcode::AcqLoad,
        dst: Some(dst),
        src: [Some(base), None],
        imm,
        predicate: 0,
...
fn rel_store_syl(base: usize, data: usize, imm: i64) -> Syllable {
    Syllable {
        opcode: Opcode::RelStore,
        dst: None,
        src: [Some(base), Some(data)],
        imm,
        predicate: 0,
...
/// AcqLoad behaves like LoadD: reads 8 bytes from memory into a GPR.
#[test]
fn acqload_reads_memory_like_load_d() {
    let mut cpu = CpuState::new(W, LatencyTable::default());

    cpu.memory[0x200] = 0x11;
    cpu.memory[0x201] = 0x22;
    cpu.memory[0x202] = 0x33;
    cpu.memory[0x203] = 0x44;
    cpu.memory[0x204] = 0x55;
    cpu.memory[0x205] = 0x66;
    cpu.memory[0x206] = 0x77;
    cpu.memory[0x207] = 0x88;

    let mut b0 = Bundle::nop_bundle(W);
    b0.set_slot(2, acq_load(1, 0, 0x200));
...
/// RelStore behaves like StoreD: writes 8 bytes from a GPR to memory.
#[test]
fn relstore_writes_memory_like_store_d() {
    let mut cpu = CpuState::new(W, LatencyTable::default());
    cpu.write_gpr(1, 0xdead_beef_cafe_1234);

    let mut b0 = Bundle::nop_bundle(W);
    b0.set_slot(2, rel_store_syl(0, 1, 0x300));
...
/// Two-CPU producer/consumer: CPU 0 writes with RelStore, CPU 1 reads with AcqLoad.
/// Memory ops must occur on cycles owned by the respective CPU:
///   CPU 0 owns even bundle indices (0, 2, 4, ...),
///   CPU 1 owns odd bundle indices (1, 3, 5, ...).
#[test]
fn two_cpu_producer_relstore_consumer_acqload() {
    let mut layout = canonical_layout(W);
    layout.topology.cpus = 2;

    // Producer (CPU 0): memory op at bundle_idx=2 (even → CPU 0 owns cycle 2)
    let mut p_b0 = Bundle::nop_bundle(W);
    p_b0.set_slot(0, mov_imm(1, 0x42)); // r1 = 0x42
    let p_b1 = Bundle::nop_bundle(W);
    let mut p_b2 = Bundle::nop_bundle(W);
    p_b2.set_slot(2, rel_store_syl(0, 1, 0x100)); // relstore [r0+0x100], r1 at cycle 2
    let p_b3 = Bundle::nop_bundle(W);
    let mut p_b4 = Bundle::nop_bundle(W);
    p_b4.set_slot(3, ret());

    // Consumer (CPU 1): memory op at bundle_idx=1 (odd → CPU 1 owns cycle 1)
    // Bundle 0 is CPU 1's cycle 0 (owned by CPU 0), so no memory here.
    let c_b0 = Bundle::nop_bundle(W); // CPU 1 cycle 0: bus belongs to CPU 0
    let mut c_b1 = Bundle::nop_bundle(W);
    c_b1.set_slot(2, acq_load(2, 0, 0x100)); // acqload r2, [r0+0x100] at cycle 1
    let c_b2 = Bundle::nop_bundle(W);
    let mut c_b3 = Bundle::nop_bundle(W);
    c_b3.set_slot(3, ret());
    ...
    assert!(system.cpus[0].halted, "producer should halt");
    assert!(system.cpus[1].halted, "consumer should halt");

    let stored = u64::from_le_bytes(system.memory.bytes()[0x100..0x108].try_into().unwrap());
    assert_eq!(stored, 0x42, "relstore should have written 0x42");
}

#[test]
fn two_cpu_write_invalidates_other_cache_line() {
    let mut layout = canonical_layout(W);
    layout.topology.cpus = 2;

    let mut cpu0_b0 = Bundle::nop_bundle(W);
    cpu0_b0.set_slot(2, rel_store_syl(0, 1, 0x100));
    let mut cpu0_b1 = Bundle::nop_bundle(W);
    cpu0_b1.set_slot(3, ret());

    let cpu1_b0 = Bundle::nop_bundle(W);
    let mut cpu1_b1 = Bundle::nop_bundle(W);
    cpu1_b1.set_slot(2, acq_load(2, 0, 0x100));
    let mut cpu1_b2 = Bundle::nop_bundle(W);
    cpu1_b2.set_slot(3, ret());

    let mut system = System::new(
        layout,
        vec![vec![cpu0_b0, cpu0_b1], vec![cpu1_b0, cpu1_b1, cpu1_b2]],
        LatencyTable::default(),
    )
    .unwrap();
    system.cpus[0].write_gpr(1, 0x55);
    system.cpus[1].cache.access_load(0x100);
    assert_eq!(system.cpus[1].cache.lines[4].msi, MsiState::Shared);

    system.step_global();

    assert_eq!(system.cpus[0].cache.lines[4].msi, MsiState::Modified);
    assert_eq!(system.cpus[1].cache.lines[4].msi, MsiState::Invalid);
    assert!(at_most_one_modified_for_system(&system, 0x100));
}

#[test]
fn two_cpu_modified_owner_moves_with_bus_serialized_store() {
    let mut layout = canonical_layout(W);
    layout.topology.cpus = 2;
    ...
    let cpu1_b0 = Bundle::nop_bundle(W);
    let mut cpu1_b1 = Bundle::nop_bundle(W);
    cpu1_b1.set_slot(2, rel_store_syl(0, 1, 0x100));
    let mut cpu1_b2 = Bundle::nop_bundle(W);
    cpu1_b2.set_slot(3, ret());
    ...
    .unwrap();
    system.cpus[0].write_gpr(1, 0x11);
    system.cpus[1].write_gpr(1, 0x22);
    ...
    system.step_global();
    assert_eq!(system.cpus[0].cache.lines[4].msi, MsiState::Invalid);
    assert_eq!(system.cpus[1].cache.lines[4].msi, MsiState::Modified);
    assert!(at_most_one_modified_for_system(&system, 0x100));
...
#[test]
fn two_cpu_coherence_fixture_pair_preserves_modified_invariant() {
    let cpu0_source =
    ...
    let mut system = System::new(
        cpu0.layout,
        vec![cpu0.bundles, cpu1.bundles],
        LatencyTable::default(),
    )
    .unwrap();

    system.step_global();
    assert_eq!(system.cpus[0].cache.lines[0].msi, MsiState::Modified);
    assert!(at_most_one_modified_for_system(&system, 0));

    system.step_global();
    assert_eq!(system.cpus[0].cache.lines[0].msi, MsiState::Invalid);
    assert_eq!(system.cpus[1].cache.lines[0].msi, MsiState::Modified);
    assert!(at_most_one_modified_for_system(&system, 0));
}

#[test]
fn worst_case_visibility_two_cpus_default_cache() {
    let mut layout = canonical_layout(W);
    layout.topology.cpus = 2;
    let vis = worst_case_visibility(&layout);
    // (2-1)*1 + miss_latency(3) + writeback_latency(0) = 4
    assert_eq!(vis, 4);
}
