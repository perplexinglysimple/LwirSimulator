...
fn check_slot_legality<const W: usize>(
    ...
    for (slot, syl) in bundle.syllables.iter().enumerate() {
        ...
        if actual != expected {
            let opname = opcode_name(syl.opcode);
            let expected_name = slot_class_name(expected);
            let actual_name = slot_class_name(actual);
            diags.push(Diagnostic {
                ...
                message: format!(
                    "bundle {bidx} slot {slot}: \
                     `{opname}` is a {actual_name} op but slot {slot} is a {expected_name} slot"
                ),
            });
        }
    }
...
fn check_gpr_hazards<const W: usize>(
    ...
    for i in 0..n {
        ...
        if !opcode_writes_gpr(ei.opcode) {
            continue;
        }
        ...
        for j in (i + 1)..n {
            ...
            if opcode_writes_gpr(lj.opcode) && lj.dst == Some(dst) {
                diags.push(Diagnostic {
                    bundle_idx: bidx,
                    slot: i,
...
fn check_pred_hazards<const W: usize>(
    ...
    for i in 0..n {
        let ei = &bundle.syllables[i];
        if !opcode_writes_pred(ei.opcode) {
            continue;
        }
        let Some(dst) = ei.dst else {
            continue;
        ...
        for j in (i + 1)..n {
            ...
            let reads_as_src = opcode_reads_pred_src(lj.opcode)
            ...
            if opcode_writes_pred(lj.opcode) && lj.dst == Some(dst) {
                diags.push(Diagnostic {
                    bundle_idx: bidx,
                    slot: i,
                    rule: Rule::SameBundlePredHazard,
...
fn update_ready_at<const W: usize>(
    ...
    for syl in &bundle.syllables {
        if opcode_writes_gpr(syl.opcode) {
...
fn opcode_writes_gpr(op: Opcode) -> bool {
    matches!(
        op,
        Opcode::Add
            | Opcode::Sub
...
fn opcode_writes_pred(op: Opcode) -> bool {
    matches!(
        op,
        Opcode::CmpEq
            | Opcode::CmpLt
...
fn opcode_reads_pred_src(op: Opcode) -> bool {
    matches!(op, Opcode::PAnd | Opcode::POr | Opcode::PXor | Opcode::PNot)
}
