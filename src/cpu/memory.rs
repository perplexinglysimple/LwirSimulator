...
impl CpuState {
    fn load8(&self, addr: usize) -> (ret: u8)
        requires self.wf(),
        ensures
            addr < self.mem_size  ==> ret == self.memory[addr as int],
            addr >= self.mem_size ==> ret == 0u8,
    {
        if addr < self.mem_size { self.memory[addr] } else { 0 }
    }

    fn load16(&self, addr: usize) -> (ret: u16)
        requires self.wf(),
        ensures
            addr + 1 < self.mem_size  ==> ret == (self.memory[addr as int] as u16)
                                         | ((self.memory[addr as int + 1] as u16) << 8),
            addr + 1 >= self.mem_size ==> ret == 0u16,
    {
        // Use addr < self.mem_size - 1 to avoid usize overflow in addr + 1.
    ...
    fn load32(&self, addr: usize) -> (ret: u32)
        requires self.wf(),
        ensures
            addr + 3 < self.mem_size  ==> ret == (self.memory[addr as int] as u32)
                | ((self.memory[addr as int + 1] as u32) << 8)
                | ((self.memory[addr as int + 2] as u32) << 16)
                | ((self.memory[addr as int + 3] as u32) << 24),
            addr + 3 >= self.mem_size ==> ret == 0u32,
    {
        if addr < self.mem_size - 3 {
    ...
    fn load64(&self, addr: usize) -> (ret: u64)
        requires self.wf(),
        ensures
            addr + 7 < self.mem_size  ==> ret == (self.memory[addr as int] as u64)
            ...
            addr + 7 >= self.mem_size ==> ret == 0u64,
    {
        if addr < self.mem_size - 7 {
    ...
    fn store8(&mut self, addr: usize, v: u8)
        ...
        ensures
            ...
            addr < self.mem_size  ==> self.memory[addr as int] == v,
            addr < self.mem_size  ==>
                forall|i: int| 0 <= i < self.mem_size && i != addr ==>
                    self.memory[i] == old(self).memory[i],
            addr >= self.mem_size ==> self.memory == old(self).memory,
    {
        if addr < self.mem_size { self.memory.set(addr, v); }
    }
    ...
    fn store16(&mut self, addr: usize, v: u16)
        ...
        ensures
            ...
            self.halted     == old(self).halted,
            self.scoreboard == old(self).scoreboard,
            addr + 1 < self.mem_size ==> self.memory[addr as int]     == (v & 0xffu16) as u8,
            addr + 1 < self.mem_size ==> self.memory[addr as int + 1] == (v >> 8) as u8,
            addr + 1 < self.mem_size ==>
                forall|i: int| 0 <= i < self.mem_size && i != addr && i != addr + 1 ==>
                    self.memory[i] == old(self).memory[i],
            addr + 1 >= self.mem_size ==> self.memory == old(self).memory,
    {
        if addr < self.mem_size - 1 {
    ...
    fn store32(&mut self, addr: usize, v: u32)
        ...
        ensures
            ...
            self.halted     == old(self).halted,
            self.scoreboard == old(self).scoreboard,
            addr + 3 < self.mem_size ==> self.memory[addr as int]     == (v & 0xffu32) as u8,
            addr + 3 < self.mem_size ==> self.memory[addr as int + 1] == ((v >>  8) & 0xffu32) as u8,
            addr + 3 < self.mem_size ==> self.memory[addr as int + 2] == ((v >> 16) & 0xffu32) as u8,
            addr + 3 < self.mem_size ==> self.memory[addr as int + 3] == (v >> 24) as u8,
            addr + 3 < self.mem_size ==>
                forall|i: int| 0 <= i < self.mem_size
                    && i != addr && i != addr+1 && i != addr+2 && i != addr+3 ==>
                    self.memory[i] == old(self).memory[i],
            addr + 3 >= self.mem_size ==> self.memory == old(self).memory,
    {
        if addr < self.mem_size - 3 {
            ...
            assert forall|i: int| 0 <= i < self.mem_size
                && i != addr && i != addr+1 && i != addr+2 && i != addr+3
    ...
    fn store64(&mut self, addr: usize, v: u64)
        ...
        ensures
            ...
            self.halted     == old(self).halted,
            self.scoreboard == old(self).scoreboard,
            addr + 7 < self.mem_size ==> self.memory[addr as int]     == (v & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 1] == ((v >>  8) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 2] == ((v >> 16) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 3] == ((v >> 24) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 4] == ((v >> 32) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 5] == ((v >> 40) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 6] == ((v >> 48) & 0xffu64) as u8,
            addr + 7 < self.mem_size ==> self.memory[addr as int + 7] == (v >> 56) as u8,
            addr + 7 < self.mem_size ==>
                forall|i: int| 0 <= i < self.mem_size
                    && i != addr   && i != addr+1 && i != addr+2 && i != addr+3
                    && i != addr+4 && i != addr+5 && i != addr+6 && i != addr+7 ==>
                    self.memory[i] == old(self).memory[i],
            addr + 7 >= self.mem_size ==> self.memory == old(self).memory,
    {
        if addr < self.mem_size - 7 {
