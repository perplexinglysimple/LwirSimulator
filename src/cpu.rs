...
impl<const W: usize> CpuState<W> {
    ...
    fn store16(&mut self, addr: usize, v: u16)
        requires old(self).memory.len() == MEM_SIZE,
        ensures
            self.memory.len() == MEM_SIZE,
    ...
    ///
    /// The body is trusted (`external_body`); these postconditions are the
    /// complete functional contract — they say exactly what each opcode does
                ...
                !self.halted,
    {
        ...
        let src0 = syl.src[0].map(|r| self.read_gpr(r)).unwrap_or(0u64);
        let src1 = syl.src[1].map(|r| self.read_gpr(r)).unwrap_or(0u64);
        ...
        match syl.opcode {
            ...
            Opcode::Shl     => self.writeback(syl, src0 << (src1 & 63), lat),
            Opcode::Srl     => self.writeback(syl, src0 >> (src1 & 63), lat),
            Opcode::Sra     => self.writeback(syl, ((src0 as i64) >> (src1 & 63)) as u64, lat),
            ...
            Opcode::LoadD   => { let a = src0.wrapping_add(imm) as usize; let v = self.load64(a); self.writeback(syl, v, lat); }
            Opcode::LoadW   => { let a = src0.wrapping_add(imm) as usize; let v = self.load32(a); self.writeback(syl, v as u64, lat); }
            Opcode::LoadH   => { let a = src0.wrapping_add(imm) as usize; let v = self.load16(a); self.writeback(syl, v as u64, lat); }
            Opcode::LoadB   => { let a = src0.wrapping_add(imm) as usize; let v = self.load8(a);  self.writeback(syl, v as u64, lat); }
            Opcode::StoreD  => { let a = src0.wrapping_add(imm) as usize; self.store64(a, src1); }
            Opcode::StoreW  => { let a = src0.wrapping_add(imm) as usize; self.store32(a, src1 as u32); }
            Opcode::StoreH  => { let a = src0.wrapping_add(imm) as usize; self.store16(a, src1 as u16); }
            Opcode::StoreB  => { let a = src0.wrapping_add(imm) as usize; self.store8(a, src1 as u8); }
            ...
            Opcode::Jump    => { self.pc = syl.imm as usize; }
            Opcode::Call    => { let rpc = self.pc; self.write_gpr(31, rpc as u64); self.pc = syl.imm as usize; }
            Opcode::Ret     => {
                let t = self.read_gpr(31) as usize;
                if t == 0 { self.halted = true; } else { self.pc = t; }
            }
            Opcode::PAnd    => {
                let a = self.read_pred(syl.src[0].unwrap_or(0));
                let b = self.read_pred(syl.src[1].unwrap_or(0));
